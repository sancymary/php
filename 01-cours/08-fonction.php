...
<div class="row">
    <h2 class="my-5">3-porter des variable dans les function</h2>
    <div class="col-sm-12 col-md-4">
        <h3 class="text-primary text-center mb-5"> variable locale</h3>
        <p>les variable declarer dans vos scripts ne sont pas accessible dans vos founction et inversement</p>
        <?php
        $a=5;
        function mafunction(){
            $b=3;
            //echo $a;//affiche variable non défini
            echo "<p>la variable \$b = $b</p>" ;//affiche 3:ici nous trouvons dans l'espace local de la function cette variable est dite locale
        }
        mafunction();
        echo "<p>la variable \$a = $a</p>";//
        //echo "<p>la variable \$b = $b</p>" ;//je demande à afficher la variable $b qui est défini dans la founction=<affiche variable ind"fini:on ne peut pas accer à cette variable car elle n'est connue que à l'interieure de la function
        ?>
    </div>

    <div class="col-sm-12 col-md-4">
        <h3 class="text-primary text-center mb-5"> variable globale</h3>
        <p>les variable declarer dans vos scripts  peut etre accessibl edans vos function à condition d'etre déclarer avec le mot clé <span>global</span> dans celle-ci</p>
        <?php
                $a=2;
                function mafunction2(){
                    global $a;//permet d'aller chercher la variable exterieure de la function pouvoir exploiter à l'interieure
                    $b=6;
                    echo "<p>la variable \$a = $a</p>" ;//affiche5
    ...
    <div class="col-sm-12 col-md-4">
        ...
        <p>les variable d'une function sont reinitialiser à chaque appelle de cette function</p>
        <p>si on veut conserver la valeure précident,il faut déclarer la variable comme statique</p>
        <?php
...
<div class="row">
    <div class="col-sm-12">
    <h2 class="my-5">4- typage de parametre le function</h2>
    <ul>
        <li>dans nos function on peut ajouter des contraintes de type sur les arguments et sur les valeure de retour de function</li>
        <li>le typage permet un debogage du code plus rapide en effect, si vous ne transmettre pas bon type de paramettre à votre function,ou si elle ne retourne pas le bon type,une erreure se déclancher immediatement au de la function,sinon vous pourrez avoir une cascade d'erreure non détecter et retournant un resumata faux</li>
    </ul>
    <?php
    function prix(int $val){
        echo"<p>cet objet coute $val euros </p>";
    }
    prix(2);
    //******************* */
    function prix3(int $val):void{//la founction attends un entiere en arrgument (int $val) et ne retourne rien void
        echo"<p>cet objet coute $val euros </p>";
    }
    prix3(3);//affiche rien de caractere avec la substitution de la caractere de la variable à l(intérieure)
    //prix(sancy)//l'appelle avec une chaine déclenche un tupeageerreure car la function attends un nombre entier en parametre 
    //on peut déclarer une union de type en ecrivant plusieure type et en les separent par des pipes
    ...
    //********************** pour le division ************/
            function div(int $a,$b){
                $resultat=$a/$b;
                return "<p> le resultat de division de la valeure $a X $b est égale à:$resultat</p> ";
                
            }
echo div(10,5);

function divise(int $no1,int $no2): int{
    echo $no1.'<br>';
    echo $no1/$no2.'<br>';
    return  $no1/$no2;
}
