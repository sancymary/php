...
$info ='';
$year =((int) date('Y'))-12;//2011
$month=( date('m'));//
$date=( date('d'));//
//date limit superieure
$dateLimitSub = $year."-".$month."-".$date;
...
if(!isset($lastName)|| strlen($lastName)< 3 || preg_match('/[0-9]+/',$lastName)){
     ...
     //le champs n'existe pas if(!isset($lastName)
...
if(!isset($civility)|| ($civility !='f' && $civility !='h')){//le champs n'existe pas if(!isset($civility)
     $info.=alert(" le civility n'est pas valide","danger");
...
if(!isset($birthday) ||  ($birthday > $dateLimitSub && $birthday < $dateLimitInf) ){
     $info.=alert(" le date de naissance n'est pas valide","danger");
...
if(empty($info)){
     ...
     if(empty($info)){
          ...
          $result=inscriptionUsers($firstname,$lastName,$pseudo,$mdp,$email,$phone,$civility,$birthday,$address,$zip,$city,$country);
